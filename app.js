...
const DOMAINS = {
  ...
  3: { name: "Design High-Performing Architectures", short: "Performance", weight: 24, color: "#3fb950" },
...
function startTimer() {
  ...
  const el = $("#timer");
  if (!state.timeLimit) { el.style.display = "none"; return; }
  ...
  tick();
  timerInterval = setInterval(() => { state.elapsed++; tick(); if (state.elapsed % 5 === 0) persist(); }, 1000);
}
...
// ---------- Navigation ----------
function next() { if (state.current < state.questions.length - 1) { state.current++; renderQuestion(); renderNav(); } }
function prev() { if (state.current > 0) { state.current--; renderQuestion(); renderNav(); } }
function toggleFlag() {
  const q = state.questions[state.current];
...
function computeResults() {
  ...
  state.questions.forEach(q => {
    ...
    const isCorrect = sel.length > 0 && cor.length === sel.length && cor.every((v,i) => v === sel[i]);
    if (isCorrect) { correctCount++; perDomain[q.domain].c++; }
    perDomain[q.domain].t++;
    details.push({ ...q, selected: state.answers[q.id] || [], isCorrect, flagged: !!state.flagged[q.id], answered: !!(state.answers[q.id] && state.answers[q.id].length) });
  });
...
function renderResults(res, auto) {
  ...
  $("#scoreRing").style.background = `conic-gradient(${ringColor} ${pct*3.6}deg, var(--border) 0deg)`;
  ...
  $("#verdictSub").textContent = (auto ? "⏱ Time's up. " : "") +
  ...
  const timeUsed = state.finishedAt ? Math.round((state.finishedAt - state.startedAt)/1000) : state.elapsed;
...
function buildPrompt(res) {
  ...
  details.forEach((d, idx) => {
    const selLabels = d.selected.length ? d.selected.map(i => letters[i]).join(",") : "(no answer)";
  ...
  lines.push("2. For EACH mistake, explains why my answer is wrong, why the correct one is right, and the underlying AWS concept I need to review;");
...
function resultsAsJson() {
  ...
  return {
    ...
    score: { correct: res.correctCount, total: res.total, percent: res.pct, passed: res.pct >= PASS_THRESHOLD },
    perDomain: Object.fromEntries(Object.keys(DOMAINS).map(d => [DOMAINS[d].short, res.perDomain[d]])),
    timeUsedSec: state.finishedAt ? Math.round((state.finishedAt - state.startedAt)/1000) : state.elapsed,
    answers: res.details.map((d, idx) => ({
      n: idx+1, id: d.id, domain: DOMAINS[d.domain].short,
      question: d.q,
      yourAnswer: d.selected.map(i => letters[i]),
      correctAnswer: d.correct.map(i => letters[i]),
      isCorrect: d.isCorrect, answered: d.answered, flagged: d.flagged,
    })),
  };
}
...
function downloadJson() {
  ...
  a.href = url; a.download = "aws-saa-results.json";
...
function retryWrong() {
  ...
  if (!wrongIds.length) { toast("No mistakes to retry 🎉"); return; }
  ...
  const questions = (cfg.shuffle ? shuffle(pool) : pool).map(q => {
    ...
    return { id:q.id, domain:q.domain, q:q.q, multi:!!q.multi,
      options: order.map(i => q.options[i]),
      correct: (Array.isArray(q.correct)?q.correct:[q.correct]).map(c=>order.indexOf(c)),
      explanation:q.explanation };
  });
  state = { config: cfg, questions, answers:{}, flagged:{}, current:0, startedAt:Date.now(), finished:false, timeLimit:0, elapsed:0 };
  persist();
  show("screen-quiz");
  startTimer();
  renderQuestion();
...
function init() {
  ...
  $("#btnRestart").onclick = () => { localStorage.removeItem(STORAGE_KEY); show("screen-start"); $("#btnResume").style.display="none"; $("#timer").style.display="none"; };
