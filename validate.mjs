...
pos.forEach((n, i) => {
  ...
  if (pct < 15 || pct > 35) err("distribution", `correct index ${i} at ${pct.toFixed(1)}% (want 15-35%)`);
});
...
const STOP = new Set("a an the to of and or in on for with that this is are be it its as at by from must needs wants company application solution which what should meets requirements aws amazon combination steps take select two solutions architect".split(" "));
const tokens = (s) => new Set(s.toLowerCase().match(/[a-z0-9-]{3,}/g)?.filter((t) => !STOP.has(t)) || []);
const sets = questions.map((q) => [q.id, tokens(q.q + " " + [q.correct].flat().map((i) => q.options[i]).join(" "))]);
for (let i = 0; i < sets.length; i++) {
  for (let j = i + 1; j < sets.length; j++) {
    const [a, A] = sets[i], [b, B] = sets[j];
...
if (auditPath) {
  ...
  const before = new Map(load(execSync("git show HEAD:questions.js", { encoding: "utf8" })).map((q) => [q.id, q]));
  const audit = JSON.parse(readFileSync(auditPath, "utf8"));
  for (const entry of audit) {
    const old = before.get(entry.id);
    if (!old) { err(entry.id, "audit entry for an id absent from HEAD"); continue; }
    const oldCorrect = [old.correct].flat().map((i) => old.options[i]).sort();
    const claimed = [entry.origCorrect].flat().sort();
    if (JSON.stringify(oldCorrect) !== JSON.stringify(claimed)) {
      err(entry.id, `audit claims original correct answer was ${JSON.stringify(claimed)}, HEAD says ${JSON.stringify(oldCorrect)}`);
    }
  }
  const rewritten = new Set(audit.map((e) => e.id));
  for (const id of before.keys()) if (!rewritten.has(id)) warn(id, "present at HEAD but no audit entry");
}

// ---------- report ----------
console.log(`${questions.length} questions`);
console.log("per domain:", Object.entries(byDomain).map(([d, n]) => `${d}:${n} (${((n / questions.length) * 100).toFixed(0)}%)`).join("  "));
console.log("per task statement:", Object.keys(TASK_STATEMENTS).map((t) => `${t}:${byTs[t] || 0}`).join("  "));
console.log("correct index:", pos.map((n, i) => `${i}:${((n / singles.length) * 100).toFixed(0)}%`).join("  "));
