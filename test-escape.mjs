...
// The declaration may span several lines, so grow the snippet until it parses.
const lines = src.split('\n');
const start = lines.findIndex((l) => l.startsWith('const esc ='));
let esc;
for (let end = start; start !== -1 && end < start + 20 && !esc; end++) {
  try {
    esc = new Function(`${lines.slice(start, end + 1).join('\n')}; return esc;`)();
  } catch {}
}
if (!esc) {
...
check(
  ...
  esc('Amazon S3 Glacier Deep Archive') === 'Amazon S3 Glacier Deep Archive',
);
